...
const RenderListItem = (props) => {
  ...
  return (
    <ListItemContext.Provider value={context}>
      <ListItem id="ListItem">{displayMode ? <DisplayComponent /> : <EditComponent />}</ListItem>
  ...
  return (
    <ListItemWrapper id="ListItemWrapper">
      ...
      <ListItemIconWrapper id="ListItemIconWrapper">
        <IconWrapper className="ListIconWrapper" onClick={() => setDisplayMode(false)}>
          <BsFillPencilFill />
        </IconWrapper>
        <IconWrapper
...
const EditComponent = () => {
  const { task, index, setDisplayMode } = useContext(ListItemContext);
  ...
  return (
    <>
      <ListItemWrapper id="ListItemWrapper">
        <EditInput value={userInput} onChange={handleUserInput} />
        <Button BackgroundColor="#000" Color="#ffffff" onClick={handleSave}>
          Save
        </Button>
      </ListItemWrapper>
      {error !== '' && error !== null && (
